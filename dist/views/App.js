...
function App() {
    ...
    var handleAddItem = function (task) {
        enterIntoArray(task, function () {
            var newItem = {
                id: task,
                item: task,
            };
            setList(__spreadArray(__spreadArray([], list), [newItem]));
        });
    };
    var handleDeleteItem = function (e, itemId) {
        var newList = list.filter(function (item) { return item.id !== itemId; });
        setList(newList);
        e.preventDefault();
    };
    var enterIntoArray = function (task, callBack) {
        var checkIfExist = function () { return list.filter(function (item) { return item.item === task; }); };
    ...
    electron_1.default.ipcRenderer.on("selected:folder", function (e, selected) {
        if (selected) {
